class MaskBlurPass extends SubPass
{
        ...
        const FragmentSource = 
           `#version 300 es
            ...
            void main()
            {
                ...
                vec4 result = texture(InputTexture, frag_uvs) * weight[0] * texture(InputTexture, frag_uvs).a;
                if (Horizontal == 0)
                {
                    for (int i = 1; i < 5; ++i)
                    {
                        result += texture(InputTexture, frag_uvs + vec2(offset.x * float(i), 0.0)) * weight[i] * ceil(texture(InputTexture, frag_uvs + vec2(offset.x * float(i), 0.0)).a);
                        result += texture(InputTexture, frag_uvs - vec2(offset.x * float(i), 0.0)) * weight[i] * ceil(texture(InputTexture, frag_uvs - vec2(offset.x * float(i), 0.0)).a);
                    }
                }
                else
                {
                    for (int i = 1; i < 5; ++i)
                    {
                        result += texture(InputTexture, frag_uvs + vec2(0.0, offset.y * float(i))) * weight[i] * ceil(texture(InputTexture, frag_uvs + vec2(0.0, offset.y * float(i))).a);
                        result += texture(InputTexture, frag_uvs - vec2(0.0, offset.y * float(i))) * weight[i] * ceil(texture(InputTexture, frag_uvs - vec2(0.0, offset.y * float(i))).a);
                    }
                }
                out_colour = vec4(result.xyz * 1.5, 1.0);
            }`
